...
def test_page_processing_independence():
    ...
    for i in range(1, 6):
        crawl_results.append({
            ...
            "text_content": f"This is the content of page {i}. " * 100,  # 每个页面约2000字符
        })
    ...
    if call_count >= len(crawl_results):
        ...
        logger.info(f"  说明每个页面都独立处理，没有合并")
        ...
        if len(unique_pages) == len(crawl_results):
            logger.info(f"✓ 所有页面都被独立处理")
            return True
        else:
            logger.warning(f"⚠ 部分页面可能未被处理或被合并")
            return False
    else:
        logger.error(f"✗ 测试失败: 大模型调用次数 ({call_count}) < 页面数 ({len(crawl_results)})")
        logger.error(f"  可能存在页面合并处理的情况")
        return False
...
def test_long_page_chunking():
    ...
    if chunk_count > 1:
        ...
        logger.info(f"  这保证了内容的完整性，避免了简单截断")
        return True
    else:
        logger.info(f"ℹ 页面未分割（可能内容在阈值内）")
        return True
